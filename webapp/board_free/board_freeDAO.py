...
class Board_freeDAO:
    ...
    def insertBoard(cls, title, content, author) -> 'Board_freeDAO':
        sql = "insert into board_free values (NULL,'" + title + "', date_format(now(),'%Y-%m-%d') ,'" + content + "','" + author + "');"
        dbc.execute(sql)
    ...
    def updateBoard(cls, board_code, title, content) -> 'Board_freeDAO':
        sql = "update board_free set bf_title = '" + title + "', bf_content = '" + content + "' where bf_num = " + board_code + ";"
        dbc.execute(sql)

    @classmethod
    def selectBoardSearchCount(cls, keyword, option) -> 'Board_freeDAO':
        sql = "select count(*) from board_free where " + option + " like '%" + keyword + "%';"
        return dbc.select(sql)
